...
class Player():
    ...
    def draw(self, app):
        if self.animationList != []:
            ...
            drawImage(currSprite, self.x-25, self.y-25)
    ...
    def setPlayerBounds(self, app):
        self.bounds = (self.x-25-5, self.y-25,
                       self.x+25+5, self.y+25)
...
def makePlayerVisible(app, player):
        ...
        if player.type == 'player': 
            ...
            if (app.player.x < app.scrollMargin):
                app.scrollX = app.player.x - app.scrollMargin
...
def lvl1_onStep(app):
    ...
    for enemy in app.lvl1enemyList:
        enemy.x -= app.scrollX
        if (((app.player.x-app.scrollX <= enemy.x and enemy.movingLeft) or
                (app.player.x-app.scrollX >= enemy.x and enemy.movingRight)) and
...
def lvl1_onKeyHold(app, keys):
    ...
    if 'space' in keys: #our 'true' conditions here (for now, logic activates when space pressed)
    #     print('pressed')
        for enemy in app.lvl1enemyList:
            # enemy.anchor -= app.scrollX
            if not enemy.playerSeen:
                if (enemy.x-app.scrollX < enemy.anchor):
                    enemy.x += 5
